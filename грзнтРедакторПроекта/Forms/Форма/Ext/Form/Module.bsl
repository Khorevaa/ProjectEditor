...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяФормыРедактирования = Обработка.Метаданные().ПолноеИмя();
...
Процедура ДатаНачалаПроектаПриИзменении(Элемент)
	...
	АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииДерева(АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, СообщениеСерверу, МассивВозврата, РежимУчитыватьФакт);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Строка") Тогда
		...
		РезультатПроверки = "";
		АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПроверитьВозможностьПеремещения(АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, СтрокаТаблицы.УИД, ВыбранноеЗначение, РезультатПроверки);
		...
		АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииДерева(АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, МассивСообщенийСерверу, МассивВозврата, РежимУчитыватьФакт);
		ОбновитьТаблицуПроектаПоМассивуИзменений(МассивВозврата, Ложь);
...
Процедура КомандаПересчетВсехСроков(Команда)
	...
	МассивВозврата = Новый Массив;
	АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетВсехСроковПересчетПоУровням(АдресВременногоХранилищаДанныхНаСервере, УникальныйИдентификатор, МассивВозврата, РежимУчитыватьФакт);
...
&НаКлиенте
Процедура КомандаЗаполнитьДемоДанные(Команда)
	Описание = Новый ОписаниеОповещения("ЗаполнитьДемоДанныеПродолжение", ЭтаФорма);
	ОткрытьФорму(ПолучитьИмяОбъектаНаКлиенте() + ".Форма.ФормаДемоДанные", , ЭтаФорма, , , , Описание);
КонецПроцедуры
...
Процедура ОтменитьДействие()
	Если СписокДействийДляИстории.Количество() > 0 Тогда
		...
		Если СтруктураВозврата.МассивСообщенийСервер.Количество() > 0 Тогда
			АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииРеквизитаБезПересчета(АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, СтруктураВозврата.МассивСообщенийСервер, РежимУчитыватьФакт);
		КонецЕсли;
...
Процедура КомандаДеревоПроектаДобавить(Команда)
	...
	АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииДерева(АдресВременногоХранилищаДанныхНаСервере, ИдентификаторФормы, СтруктураСообщенияНаСервер, МассивИзменений, РежимУчитыватьФакт);
...
Процедура КомандаДеревоПроектаСкопироватьПродолжение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	МассивИзменений = Новый Массив;
	АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииДерева(АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, МассивСообщенийСерверу, МассивИзменений, РежимУчитыватьФакт);
	ОбновитьТаблицуПроектаПоМассивуИзменений(МассивИзменений, Ложь);
...
Процедура КомандаСвязатьВыделенныеЗадачиПродолжение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если МассивПредшественники.Количество() > 0 Тогда
		...
		АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПроверитьВозможностьЗаменыПредшественников(АдресВременногоХранилищаДанныхНаСервере, УникальныйИдентификатор, МассивТекущие, МассивПредшественники, РезультатПроверки);
	...
	Если СписокСообщенийСерверу.Количество() > 0 Тогда
		МассивВозврата = Новый Массив;
		АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииДерева(АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, СписокСообщенийСерверу.ВыгрузитьЗначения(), МассивВозврата, РежимУчитыватьФакт);
		ОбновитьТаблицуПроектаПоМассивуИзменений(МассивВозврата, Истина);
	КонецЕсли;
...
Процедура ДеревоПроектаРучнойВводПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ФактическаяДатаНачала = Дата(1,1,1) И ТекущиеДанные.ФактическаяДатаОкончания = Дата(1,1,1) Тогда
		...
		МассивВозврата = Новый Массив;
		АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииСроков(АдресВременногоХранилищаДанныхНаСервере, УникальныйИдентификатор, СообщениеСерверу, МассивВозврата, РежимУчитыватьФакт);
		ОбновитьТаблицуПроектаПоМассивуИзменений(МассивВозврата, Ложь);
	КонецЕсли;
	ЭтаФорма.Модифицированность = Истина;
...
Процедура ДеревоПроектаДатаНачалаПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ФактическаяДатаНачала = Дата(1,1,1) Тогда
		...
		МассивИзменений = Новый Массив;
		АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииСроков(АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, СтруктураИзменения, МассивИзменений, РежимУчитыватьФакт);
		ОбновитьТаблицуПроектаПоМассивуИзменений(МассивИзменений, Ложь);
	КонецЕсли;
	ЭтаФорма.Модифицированность = Истина;
КонецПроцедуры
...
Процедура ДеревоПроектаЕдиницаВремениПриИзменении(Элемент)
	...
	МассивИзменений = Новый Массив;
	АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииСроков(АдресВременногоХранилищаДанныхНаСервере, УникальныйИдентификатор, СтруктураИзменения, МассивИзменений, РежимУчитыватьФакт);
	ОбновитьТаблицуПроектаПоМассивуИзменений(МассивИзменений, Ложь);
...
Процедура СформироватьТаблицуПроектаПоЭкземпляруНаСервере()
	...
	Если НЕ ЗначениеЗаполнено(ЭкземплярВерсии) Тогда
		...
		грзнтРекурсивныйПересчет.РекурсивныйПересчетПостроитьТаблицыРасчета(ТаблицаПроектаКопия.Выгрузить(), ТаблицаПредшественники.Выгрузить(), ДатаНачалаПроекта, КалендарьПроекта, СпособПланирования, ЭтаФорма.УникальныйИдентификатор, РежимУчитыватьФакт);
	...
	АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПостроитьТаблицыРасчета(ТаблицаПроектаКопия.Выгрузить(,СтрокаКолонок), ТаблицаПредшественники.Выгрузить(), ДатаНачалаПроекта, КалендарьПроекта, СпособПланирования, ЭтаФорма.УникальныйИдентификатор, РежимУчитыватьФакт);
...
Функция ПереместитьСтрокуДереваСДРВправо(СтрокаТаблицы, НачалиДействие)
	...
	РезультатПроверки = "";
	ЭтаФорма.АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПроверитьВозможностьПеремещения(ЭтаФорма.АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, СтрокаТаблицы.УИД, НовыйРодитель.УИД, РезультатПроверки);
	Если НЕ ПустаяСтрока(РезультатПроверки) Тогда
		Сообщение = Новый СообщениеПользователю;
...
Процедура КомандаПанельПроектаУдалитьСтрокуПлана(Команда)
	...
	ИначеЕсли Элементы.ГруппаСтраницыПанельЗадачи.ТекущаяСтраница = Элементы.СтраницаПланМатериалов Тогда
		...
		Если НЕ Отказ Тогда
			...
			ТаблицаПланМатериаловОбщаяПослеУдаления(Элементы.ТаблицаПланМатериаловОбщая);
		КонецЕсли;
...
Процедура ДеревоПроектаПроцентГотовностиПанельЗадачПриИзменении(Элемент)
	...
	Если СтрокаТаблицы.ВидЭлемента <> ПредопределенноеЗначение("Справочник.грзнтВидыЭлементовПроекта.ГруппаЗадач") Тогда
		...
		МассивИзменений = Новый Массив;
		АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииСроков(АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, МассивСообщенийСерверу, МассивИзменений, РежимУчитыватьФакт);
		ОбновитьТаблицуПроектаПоМассивуИзменений(МассивИзменений, Ложь);
	КонецЕсли;
	ПосчитатьИтогиПроцентовПоКолонкам("", );
...
Процедура ДеревоПроектаФактическаяДатаНачалаПанельЗадачПриИзменении(Элемент)
	...
	МассивВозврата = Новый Массив;
	АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииСроков(АдресВременногоХранилищаДанныхНаСервере, УникальныйИдентификатор, МассивСообщенийСерверу, МассивВозврата, РежимУчитыватьФакт);
	ОбновитьТаблицуПроектаПоМассивуИзменений(МассивВозврата, Ложь);
...
Процедура ТаблицаПредшественникиНаименованиеЗадачиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если НЕ ПустаяСтрока(ТекущиеДанные.ПредшественникУИД) Тогда
		СписокУИДПредшественикиДляПересчетаПоследователи.Добавить(ТекущиеДанные.ПредшественникУИД);
	КонецЕсли;
...
&НаКлиенте
Процедура ТаблицаПланРолейОбщаяВидСтавкиПриИзменении(Элемент)
	ТекущиеДанные = Элементы.ТаблицаПланРолейОбщая.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ТаблицаПланРесурсовОбщаяТрудовойРесурсНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	КонецЕсли;
	СтандартнаяОбработка = Ложь;
	ОткрытьФорму("Справочник.грзнтТрудовыеРесурсы.Форма.ФормаВыбораСОтборомПоРоли", Новый Структура("Отбор", Новый Структура("Роль", ТекущиеДанные.Роль)), Элемент);
КонецПроцедуры
...
Процедура ТаблицаПланМатериаловОбщаяВидСтавкиНачалоВыбораПродолжение(ЗначениеВыбора, ДополнительныеПараметры) Экспорт
	...
	КонецЕсли;
	ТекущиеДанные.ВидСтавки = ЗначениеВыбора.Значение;
	ПересчитатьСуммуПоВидуСтавки(ТекущиеДанные, Неопределено, ТекущиеДанные.Номенклатура, ТекущиеДанные.ЕдиницаИзмерения);
...
Процедура СкопироватьПланУзла(ПриемникУИД, ИсточникУИД, ВидПлана)
	...
	Если ВидПлана = "ТаблицаПредшественники" Тогда
		...
		// ((( grznt.Solntsev 27.01.2017
		Для Каждого СтрокаПланов ИЗ СтрокаПриемник.ТаблицаПредшественники Цикл
			ОписаниеСобытия = Новый Структура;
			ОписаниеСобытия.Вставить("Событие", "ИзменениеПлана");
			ОписаниеСобытия.Вставить("ИмяТаблицы", "ТаблицаПредшественники");
			ОписаниеСобытия.Вставить("СтрокаТаблицы", СтрокаПриемник);
			ОписаниеСобытия.Вставить("СобытиеПлана", "УдалениеСтроки");
			ОписаниеСобытия.Вставить("ГУИД", СтрокаПланов.ГУИД);
			СтруктураПлана = Новый Структура(Новый ФиксированнаяСтруктура(СтруктураПланПредшественники));
			ЗаполнитьЗначенияСвойств(СтруктураПлана, СтрокаПланов);
			ОписаниеСобытия.Вставить("СтрокаПлана", СтруктураПлана);
			ДобавитьСобытиеТаблицы(ОписаниеСобытия);
		КонецЦикла;
		//ОписаниеСобытия = Новый Структура;
		...
		Для Каждого СтрокаПланов ИЗ СтрокаИсточник.ТаблицаПредшественники Цикл
			...
			НоваяСтрокаПланов.ГУИД = Новый УникальныйИдентификатор;
			...
			ОписаниеСобытия.Вставить("СтрокаТаблицы", СтрокаПриемник);
			ОписаниеСобытия.Вставить("СобытиеПлана", "ДобавлениеСтроки");
			ОписаниеСобытия.Вставить("ГУИД", НоваяСтрокаПланов.ГУИД);
			ДобавитьСобытиеТаблицы(ОписаниеСобытия);
			// ))) grznt.Solntsev 27.01.2017
	...
	Иначе
		// ((( grznt.Solntsev 27.01.2017
		Если ВидПлана = "ПланДвиженияДенежныхСредств" Тогда
			СтруктураПланаШаблон = СтруктураПланДДС;
		ИначеЕсли ВидПлана = "ПланДоходов" Тогда
			СтруктураПланаШаблон = СтруктураПланДоходы;
		ИначеЕсли ВидПлана = "ПланМатериаловОбщая" Тогда
			СтруктураПланаШаблон = СтруктураПланМатериалы;
		ИначеЕсли ВидПлана = "ПланРасходов" Тогда
			СтруктураПланаШаблон = СтруктураПланРасходы;
		ИначеЕсли ВидПлана = "ПланРесурсовОбщая" Тогда
			СтруктураПланаШаблон = СтруктураПланРесурсы;
		ИначеЕсли ВидПлана = "ПланРолейОбщая" Тогда
			СтруктураПланаШаблон = СтруктураПланРоли;
		КонецЕсли;
		Для Каждого СтрокаПланов ИЗ СтрокаПриемник[ВидПлана] Цикл
			ОписаниеСобытия = Новый Структура;
			ОписаниеСобытия.Вставить("Событие", "ИзменениеПлана");
			...
			ОписаниеСобытия.Вставить("СобытиеПлана", "УдалениеСтроки");
			ОписаниеСобытия.Вставить("ГУИД", СтрокаПланов.ГУИД);
			СтруктураПлана = Новый Структура(Новый ФиксированнаяСтруктура(СтруктураПланаШаблон));
			ЗаполнитьЗначенияСвойств(СтруктураПлана, СтрокаПланов);
			ОписаниеСобытия.Вставить("СтрокаПлана", СтруктураПлана);
			ДобавитьСобытиеТаблицы(ОписаниеСобытия);
		КонецЦикла;
...
Процедура ЗаменитьУИДВоВсехТаблицах(СтарыйУИД, НовыйУИД)
	...
	МассивВозврата = Новый Массив;
	АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииСроков(АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, СообщениеСерверу, МассивВозврата, РежимУчитыватьФакт);
	ОбновитьТаблицуПроектаПоМассивуИзменений(МассивВозврата, Истина);
...
Процедура ИзменитьДанныеЗадачиНаКлиентеИзСтруктуры(СтруктураОповещения)
	...
	ИначеЕсли МассивСообщенийСерверу.Количество() > 0 Тогда
		МассивВозврата = Новый Массив;
		АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииСроков(АдресВременногоХранилищаДанныхНаСервере, ЭтаФорма.УникальныйИдентификатор, МассивСообщенийСерверу, МассивВозврата, РежимУчитыватьФакт);
		ОбновитьТаблицуПроектаПоМассивуИзменений(МассивВозврата, Истина);
	КонецЕсли;
...
Функция ПолучитьИмяОбъектаНаКлиенте() Экспорт
	...
	//Сообщить("Не забудь переделать получение имя объекта!");
...
Процедура ПересчитатьСуммуПоВидуСтавки(ТекущиеДанные, Роль, Ресурс, ЕдиницаИзмерения)
	...
	СтруктураДанные.Вставить("Роль", ?(Роль = Неопределено, ПредопределенноеЗначение("Справочник.грзнтРоли.ПустаяСсылка"), Роль));
...
Процедура ДобавитьЭлементыСтруктурыПроекта(СтруктураВыбора)
	...
	Если СтруктураВыбора.ПереноситьСвязи Тогда
		МассивПредшественники = Новый Массив;
		Для Каждого СтруктураПлана ИЗ СтруктураВыбора.Предшественники Цикл
			СтрокаТаблицыПроекта = тСоответствиеТаблицыУИД.Получить(СоответствиеНовыеУИД.Получить(СтруктураПлана.Элемент));
			СтрокаПредшественник = тСоответствиеТаблицыУИД.Получить(СоответствиеНовыеУИД.Получить(СтруктураПлана.Предшественник));
			Если СтрокаТаблицыПроекта = Неопределено ИЛИ СтрокаПредшественник = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			СтруктураПредшественник = Новый Структура;
			СтруктураПредшественник.Вставить("УИД", СтрокаТаблицыПроекта.УИД);
			СтруктураПредшественник.Вставить("ПредшественникУИД", СтрокаПредшественник.УИД);
			СтруктураПредшественник.Вставить("ПредшественникНаименование", СтрокаПредшественник.НаименованиеЗадачи);
			СтруктураПредшественник.Вставить("ПредшественникКодСДР", СтрокаПредшественник.КодСДР);
			СтруктураПредшественник.Вставить("ПредшественникНомерСтрокиПроекта", СтрокаПредшественник.НомерСтрокиПроекта);
			СтруктураПредшественник.Вставить("Задержка", СтруктураПлана.Задержка);
			СтруктураПредшественник.Вставить("ЕдиницаВремени", СтруктураПлана.ЕдиницаВремени);
			СтруктураПредшественник.Вставить("ТипСвязи", ПредопределенноеЗначение("Перечисление.грзнтТипСвязи.ОкончаниеНачало"));
		...
		Если МассивПредшественники.Количество() > 0 Тогда
			...
			АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПроверитьВозможностьЗаменыПредшественников(АдресВременногоХранилищаДанныхНаСервере, УникальныйИдентификатор, МассивТекущие, МассивПредшественники, РезультатПроверки);
			Если РезультатПроверки = Неопределено Тогда
				МассивСообщенийСерверу.Очистить();
				// Если проверка отработала и не выдало предупреждения, то продолжим
				Для Каждого СтруктураПредшественник ИЗ МассивПредшественники Цикл
					СтрокаТаблицыПроекта = тСоответствиеТаблицыУИД.Получить(СтруктураПредшественник.УИД);
					СтрокаПредшественник = СтрокаТаблицыПроекта.ТаблицаПредшественники.Добавить();
					СтрокаПредшественник.НаименованиеЗадачи = СтруктураПредшественник.ПредшественникНаименование;
					...
					Если СтрокаТаблицыПроекта.Идентификатор >= 0 Тогда
						...
						СтрокаДерева.ТаблицаПредшественники.Очистить();
						Для Каждого СтрокаПлан ИЗ СтрокаТаблицыПроекта.ТаблицаПредшественники Цикл
							НоваяСтрокаПлан = СтрокаДерева.ТаблицаПредшественники.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрокаПлан, СтрокаПлан);
						КонецЦикла;
					КонецЕсли;
...
Функция ДополнитьСтруктуруВыбораДаннымиПереносаЭлементаСтруктурыПроекта(СтруктураВыбора)
	...
	ЗапросПланы.Текст =
	"
	|" + ?(СтруктураВыбора.ПереноситьСрокиИДлительность, "ВЫБРАТЬ РАЗРЕШЕННЫЕ РегистрДлительность." + ИмяПредшественник + " КАК Элемент, РегистрДлительность.ПлановыеТрудозатратыДлительность КАК Длительность, РегистрДлительность.ПлановыеТрудозатратыЕдиницаВремени КАК ЕдиницаВремени ИЗ РегистрСведений." + ИмяРегистра + ".СрезПоследних КАК РегистрДлительность ГДЕ РегистрДлительность." + ИмяПредшественник + " В ИЕРАРХИИ(&ЭлементСправочника)", "ВЫБРАТЬ 0 ГДЕ ЛОЖЬ") + ";
	|" + ?(СтруктураВыбора.ПереноситьСвязи, "ВЫБРАТЬ РАЗРЕШЕННЫЕ СправочникПланПредшественники.Ссылка КАК Элемент, СправочникПланПредшественники.* ИЗ Справочник." + ИмяСправочника + "." + ИмяПредшественники + " КАК СправочникПланПредшественники ГДЕ СправочникПланПредшественники.Ссылка В ИЕРАРХИИ (&ЭлементСправочника)", "ВЫБРАТЬ 0 ГДЕ ЛОЖЬ") + ";
	...
	МассивЗадачДобавить.Добавить(СтрокаТаблицы);
	Пока МассивЗадачДобавить.Количество() > 0 Цикл
		СтрокаИсточник = МассивЗадачДобавить.Получить(0);
		СтрокаТаблицы = ТаблицаЗадач.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицы, СтрокаИсточник, "Элемент, ПолноеНаименование");
		// Принудительно очистим выбранному родителя (чтобы знать корень, какую мы импортировали)
		СтрокаРодитель = ТаблицаПроектаИсточник.Найти(СтрокаИсточник.Родитель, "Элемент");
		...
		Если СтруктураВыбора.ПереноситьЗадачаВместеСПодчиненными Тогда
			СтрокиПодчиненные = ТаблицаПроектаИсточник.НайтиСтроки(Новый Структура("Родитель", СтрокаИсточник.Элемент));
	...
	Если СтруктураВыбора.ПереноситьСрокиИДлительность Тогда
		...
		Для Каждого ЭлементСпискаПроекта ИЗ СписокЗадач Цикл
			...
			Для Каждого СтрокаПереноса ИЗ СтрокиПереноса Цикл
				СтруктураЗаписи = Новый Структура(Новый ФиксированнаяСтруктура(СтруктураПереноса));
				ЗаполнитьЗначенияСвойств(СтруктураЗаписи, СтрокаПереноса);
				СтруктураЗаписи.ЕдиницаВремени = ?(СтрокаПереноса.ЕдиницаВремени.Пустая(), ПредопределенноеЗначение("Справочник.грзнтЕдиницыВремени.РабочийДень"), СтрокаПереноса.ЕдиницаВремени);
				МассивПлана.Добавить(СтруктураЗаписи);
			КонецЦикла;
		КонецЦикла;
		СтруктураВыбора.Вставить("ПлановыеСроки", МассивПлана);
	...
	Если СтруктураВыбора.ПереноситьДДС Тогда
		МассивПлана = Новый Массив;
		// В регистре хранится реквизит "Статья", а в таблице "СтатьяДДС", для единообразного заполнения используем оба реквизита
	...
	Если СтруктураВыбора.ПереноситьСвязи Тогда
		МассивПлана = Новый Массив;
		СтруктураПереноса = Новый Структура("Элемент, Предшественник, Задержка, ЕдиницаВремени, Календарь");
		ТаблицаПереноса = МассивРезультатыПланы.Получить(1).Выгрузить();
		Для Каждого ЭлементСпискаПроекта ИЗ СписокЗадач Цикл
			СтрокиПереноса = ТаблицаПереноса.НайтиСтроки(Новый Структура("Элемент", ЭлементСпискаПроекта.Значение.Элемент));
			Для Каждого СтрокаПереноса ИЗ СтрокиПереноса Цикл
				СтруктураЗаписи = Новый Структура(Новый ФиксированнаяСтруктура(СтруктураПереноса));
				ЗаполнитьЗначенияСвойств(СтруктураЗаписи, СтрокаПереноса);
				СтруктураЗаписи.Предшественник = СтрокаПереноса[ИмяПредшественник];
				МассивПлана.Добавить(СтруктураЗаписи);
			КонецЦикла;
		КонецЦикла;
...
&НаКлиенте
Процедура ЗаполнитьДемоДанныеПродолжение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(РезультатЗакрытия) <> Тип("Структура") Тогда
		Возврат;
		РезультатЗакрытия = Новый Структура;
	КонецЕсли;
	Если НЕ РезультатЗакрытия.Свойство("БазоваяДлительность") ИЛИ НЕ РезультатЗакрытия.Свойство("ВсегоРесурсов") ИЛИ НЕ РезультатЗакрытия.Свойство("ВсегоРолей") 
		ИЛИ НЕ РезультатЗакрытия.Свойство("КоличествоЗадач") ИЛИ НЕ РезультатЗакрытия.Свойство("КоличествоСтрокВыплат") ИЛИ НЕ РезультатЗакрытия.Свойство("КоличествоСтрокДоходов")
		ИЛИ НЕ РезультатЗакрытия.Свойство("КоличествоСтрокПоступлений") ИЛИ НЕ РезультатЗакрытия.Свойство("КоличествоСтрокРасходов") ИЛИ НЕ РезультатЗакрытия.Свойство("ОбщаяСуммаПланДоходов")
		ИЛИ НЕ РезультатЗакрытия.Свойство("ОбщаяСуммаПланРасходов") ИЛИ НЕ РезультатЗакрытия.Свойство("ОбщееКоличествоНоменклатуры") ИЛИ НЕ РезультатЗакрытия.Свойство("ОбщееКоличествоЧасов")
		ИЛИ НЕ РезультатЗакрытия.Свойство("СоздаватьНовые") Тогда
		Возврат;
	КонецЕсли;
	
	МассивНовыхЗадач = Новый Массив;
	МассивСообщенийНаСервер = Новый Массив;
	Генератор = Новый ГенераторСлучайныхЧисел(ТекущаяДата() - Дата(1,1,1));
	
	НачатьДействие();
	
	Если РезультатЗакрытия.СоздаватьНовые Тогда
		Для К = 1 ПО РезультатЗакрытия.КоличествоЗадач Цикл
			НовыйЭлемент = ДеревоПроекта.ПолучитьЭлементы().Добавить();
			ЗаполнитьНачальныеЗначенияСтроки(НовыйЭлемент);
			НовыйЭлемент.Длительность = Окр(РезультатЗакрытия.БазоваяДлительность * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
			НовыйЭлемент.НаименованиеЗадачи = "Задача №" + К;
			...
			ЗаполнитьЗначенияСвойств(СтруктураДанныхЗадачи, НовыйЭлемент);
			СтруктураСообщенияНаСервер.Вставить("ДанныеЗадачи", СтруктураДанныхЗадачи);
			МассивСообщенийНаСервер.Добавить(СтруктураСообщенияНаСервер);
			
			Если МассивНовыхЗадач.Количество() > 0 Тогда
				// Добавить рандом
				СлучайныйИндекс = Генератор.СлучайноеЧисло(0, МассивНовыхЗадач.Количество());
				Если СлучайныйИндекс > 0 Тогда
					Предшественник = МассивНовыхЗадач.Получить(СлучайныйИндекс - 1);

					СтрокаПредшественник = НовыйЭлемент.ТаблицаПредшественники.Добавить();
					СтрокаПредшественник.НаименованиеЗадачи = Предшественник.НаименованиеЗадачи;
					СтрокаПредшественник.Внешний = Ложь;
					СтрокаПредшественник.ТипСвязи = ПредопределенноеЗначение("Перечисление.грзнтТипСвязи.ОкончаниеНачало");
					СтрокаПредшественник.ПредшественникУИД = Предшественник.УИД;
					СтрокаПредшественник.Задержка = 0;
					СтрокаПредшественник.ЕдиницаВремени = ПредопределенноеЗначение("Справочник.грзнтЕдиницыВремени.РабочийДень");
					СтрокаПредшественник.КодСДР = "";
					СтрокаПредшественник.НомерСтрокиПроекта = "";
					СтрокаПредшественник.ГУИД = Строка(Новый УникальныйИдентификатор);
					
					НоваяСтрокаПланов = СтрокаТаблицыПроекта.ТаблицаПредшественники.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаПланов, СтрокаПредшественник);
					...
					НоваяСтрокаПланов = ТаблицаПредшественники.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаПланов, СтрокаПредшественник);
					НоваяСтрокаПланов.УИД = НовыйЭлемент.УИД;
					
					СообщениеСерверу = Новый Структура;
					СообщениеСерверу.Вставить("ИмяСобытия", "ДобавитьПредшественника");
					СообщениеСерверу.Вставить("УИД", СтрокаТаблицыПроекта.УИД);
					ДанныеЗадачи = Новый Структура("УИД, ПредшественникУИД, Задержка, ЕдиницаВремени, ТипСвязи");
					ЗаполнитьЗначенияСвойств(ДанныеЗадачи, НоваяСтрокаПланов);
					СообщениеСерверу.Вставить("ДанныеЗадачи", ДанныеЗадачи);
					МассивСообщенийНаСервер.Добавить(СообщениеСерверу);
				...
				КонецЕсли;
			КонецЕсли;
			МассивНовыхЗадач.Добавить(НовыйЭлемент);
		КонецЦикла;
		
		МассивИзменений = Новый Массив;
		ИдентификаторФормы = ЭтаФорма.УникальныйИдентификатор;
		АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПриИзмененииДерева(АдресВременногоХранилищаДанныхНаСервере, ИдентификаторФормы, МассивСообщенийНаСервер, МассивИзменений, РежимУчитыватьФакт);
		ОбновитьТаблицуПроектаПоМассивуИзменений(МассивИзменений, Ложь);
			
		ПересчитатьСДРДерева(, Истина);
	Иначе
		Для К = 1 ПО РезультатЗакрытия.КоличествоЗадач Цикл
			ИндексРандом = Генератор.СлучайноеЧисло(0, ТаблицаПроектаКопия.Количество() - 1);
			Пока ТаблицаПроектаКопия.Получить(ИндексРандом).Внешний Цикл
				ИндексРандом = Генератор.СлучайноеЧисло(0, ТаблицаПроектаКопия.Количество() - 1);
			КонецЦикла;
			МассивНовыхЗадач.Добавить(ДеревоПроекта.НайтиПоИдентификатору(ТаблицаПроектаКопия.Получить(ИндексРандом).Идентификатор));
		КонецЦикла;
	КонецЕсли;
	
	МассивРолей = ПолучитьМассивРолейДляДемо();
	МассивРесурсов = ПолучитьМассивРесурсовДляДемо();
	МассивНоменклатуры = ПолучитьМассивНоменклатурыДляДемо();
	МассивОрганизаций = ПолучитьМассивОрганизацийДляДемо();
	МассивПодразделений = ПолучитьМассивПодразделенийДляДемо();
	МассивСтатейЗатрат = ПолучитьМассивСтатейЗатратДляДемо();
	МассивСтатейДоходов = ПолучитьМассивСтатейДоходовДляДемо();
	МассивСтатейДДС = ПолучитьМассивСтатейДДСДляДемо();
	МассивЗонРасчета = ПолучитьМассивЗонРасчетаДляДемо();
	МассивПрофилейРаспределения = Новый Массив;
	МассивПрофилейРаспределения.Добавить(ПредопределенноеЗначение("Справочник.грзнтПрофилиРаспределения.НаКонецПериода"));
	МассивПрофилейРаспределения.Добавить(ПредопределенноеЗначение("Справочник.грзнтПрофилиРаспределения.НаНачалоПериода"));
	МассивПрофилейРаспределения.Добавить(ПредопределенноеЗначение("Справочник.грзнтПрофилиРаспределения.РавномерноПоДням"));
	//++++++++++++++++++++++++++++++++++++++
	// План доходов
	Если РезультатЗакрытия.ОбщаяСуммаПланДоходов > 0 Тогда
		СуммаДоходовВсего = РезультатЗакрытия.ОбщаяСуммаПланДоходов;
		СуммаДоходовНаКаждую = Окр(СуммаДоходовВсего / МассивНовыхЗадач.Количество(), 0);
		МассивСумм = Новый Массив;
		Для К = 1 ПО МассивНовыхЗадач.Количество() Цикл
			СуммаДоходаНаЗадачу = Окр(СуммаДоходовНаКаждую * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
			МассивСумм.Добавить(СуммаДоходаНаЗадачу);
			СуммаДоходовВсего = СуммаДоходовВсего - СуммаДоходаНаЗадачу;
		КонецЦикла;
		Если СуммаДоходовВсего <> 0 И МассивНовыхЗадач.Количество() > 0 Тогда
			МассивСумм.Установить(0, МассивСумм.Получить(0) + СуммаДоходовВсего);
		КонецЕсли;
		Если РезультатЗакрытия.КоличествоСтрокДоходов = 0 Тогда
			РезультатЗакрытия.КоличествоСтрокДоходов = 1;
		КонецЕсли;
		Если РезультатЗакрытия.КоличествоСтрокПоступлений = 0 Тогда
			РезультатЗакрытия.КоличествоСтрокПоступлений = 1;
		КонецЕсли;
		Для К = 0 ПО МассивНовыхЗадач.Количество() - 1 Цикл
			ЭлементДерева = МассивНовыхЗадач.Получить(К);
			СтрокаТаблицыПроекта = тСоответствиеТаблицыУИД.Получить(ЭлементДерева.УИД);
			
			МассивСуммПлана = Новый Массив;
			СуммаДоходовВсего = МассивСумм.Получить(К);
			СуммаДоходовНаКаждую = Окр(СуммаДоходовВсего / РезультатЗакрытия.КоличествоСтрокДоходов, 0);
			Для Ц = 1 ПО РезультатЗакрытия.КоличествоСтрокДоходов Цикл
				СуммаДоходаНаСтроку = Окр(СуммаДоходовНаКаждую * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
				МассивСуммПлана.Добавить(СуммаДоходаНаСтроку);
				СуммаДоходовВсего = СуммаДоходовВсего - СуммаДоходаНаСтроку;
			КонецЦикла;
			Если СуммаДоходовВсего <> 0 И МассивСуммПлана.Количество() > 0 Тогда
				МассивСуммПлана.Установить(0, МассивСуммПлана.Получить(0) + СуммаДоходовВсего);
			КонецЕсли;
			
			Для Ц = 0 ПО РезультатЗакрытия.КоличествоСтрокДоходов - 1 Цикл
				СтрокаПлана = ЭлементДерева.ПланДоходов.Добавить();
				СтрокаПлана.ГУИД = Новый УникальныйИдентификатор;
				СтрокаПлана.Сумма = МассивСуммПлана.Получить(Ц);
				СтрокаПлана.Валюта = ВалютаУчета;
				СтрокаПлана.СуммаУпр = СтрокаПлана.Сумма;
				
				СтрокаПлана.Статья = ПолучитьПроизвольныйЭлементМассива(МассивСтатейДоходов, Генератор);
				СтрокаПлана.Организация = ПолучитьПроизвольныйЭлементМассива(МассивОрганизаций, Генератор);
				СтрокаПлана.Подразделение = ПолучитьПроизвольныйЭлементМассива(МассивПодразделений, Генератор);
				СтрокаПлана.Номенклатура = ПолучитьПроизвольныйЭлементМассива(МассивНоменклатуры, Генератор);
				СтрокаПлана.ЗонаРасчета = ПолучитьПроизвольныйЭлементМассива(МассивЗонРасчета, Генератор);
				
				//СлучайноеЧисло = Генератор.СлучайноеЧисло(1, 2);
				//Если СлучайноеЧисло = 1 Тогда 
				//	СтрокаПлана.СпособПривязки = ПредопределенноеЗначение("Перечисление.грзнтСпособПривязки.НаДату");
				//	СтрокаПлана.ДатаПривязки = ЭлементДерева.ДатаНачала + 43200 * ЭлементДерева.Длительность;
				//Иначе
					СтрокаПлана.СпособПривязки = ПредопределенноеЗначение("Перечисление.грзнтСпособПривязки.ПоПрофилюРаспределения");
					СтрокаПлана.ПрофильРаспределения = ПолучитьПроизвольныйЭлементМассива(МассивПрофилейРаспределения, Генератор);
					//СлучайноеЧисло = Генератор.СлучайноеЧисло(1, 3);
					//Если СлучайноеЧисло = 1 Тогда
					//	СтрокаПлана.ПрофильРаспределения = ПредопределенноеЗначение("Справочник.грзнтПрофилиРаспределения.НаКонецПериода");
					//ИначеЕсли СлучайноеЧисло = 1 Тогда
					//	СтрокаПлана.ПрофильРаспределения = ПредопределенноеЗначение("Справочник.грзнтПрофилиРаспределения.РавномерноПоДням");
					//Иначе
					//	СтрокаПлана.ПрофильРаспределения = ПредопределенноеЗначение("Справочник.грзнтПрофилиРаспределения.НаНачалоПериода");
					//КонецЕсли;
				//КонецЕсли;
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПроекта.ПланДоходов.Добавить(), СтрокаПлана);
			КонецЦикла;
			
			МассивСуммПлана = Новый Массив;
			СуммаДоходовВсего = МассивСумм.Получить(К);
			СуммаДоходовНаКаждую = Окр(СуммаДоходовВсего / РезультатЗакрытия.КоличествоСтрокПоступлений, 0);
			Для Ц = 1 ПО РезультатЗакрытия.КоличествоСтрокПоступлений Цикл
				СуммаДоходаНаСтроку = Окр(СуммаДоходовНаКаждую * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
				МассивСуммПлана.Добавить(СуммаДоходаНаСтроку);
				СуммаДоходовВсего = СуммаДоходовВсего - СуммаДоходаНаСтроку;
			КонецЦикла;
			...
			Для Ц = 0 ПО РезультатЗакрытия.КоличествоСтрокПоступлений - 1 Цикл
				СтрокаПлана = ЭлементДерева.ПланДвиженияДенежныхСредств.Добавить();
				СтрокаПлана.ГУИД = Новый УникальныйИдентификатор;
				СтрокаПлана.Направление = ПредопределенноеЗначение("Перечисление.грзнтНаправлениеДвиженияДенежныхСредств.Поступление");
				СтрокаПлана.Сумма = МассивСуммПлана.Получить(Ц);
				СтрокаПлана.Валюта = ВалютаУчета;
				СтрокаПлана.СуммаУпр = СтрокаПлана.Сумма;
				...
				СтрокаПлана.Подразделение = ПолучитьПроизвольныйЭлементМассива(МассивПодразделений, Генератор);
				СтрокаПлана.Номенклатура = ПолучитьПроизвольныйЭлементМассива(МассивНоменклатуры, Генератор);
				СтрокаПлана.СтатьяДвиженияДенежныхСредств = ПолучитьПроизвольныйЭлементМассива(МассивСтатейДДС, Генератор);
				СтрокаПлана.ЗонаРасчета = ПолучитьПроизвольныйЭлементМассива(МассивЗонРасчета, Генератор);
				...
				//Иначе
					...
					//КонецЕсли;
				//КонецЕсли;
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПроекта.ПланДвиженияДенежныхСредств.Добавить(), СтрокаПлана);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	...
	//++++++++++++++++++++++++++++++++++++++
	// План расходов
	Если РезультатЗакрытия.ОбщаяСуммаПланРасходов > 0 Тогда
		СуммаРасходовВсего = РезультатЗакрытия.ОбщаяСуммаПланРасходов;
		СуммаРасходовНаКаждую = Окр(СуммаРасходовВсего / МассивНовыхЗадач.Количество(), 0);
		МассивСумм = Новый Массив;
		СуммаЧасовВсего = РезультатЗакрытия.ОбщееКоличествоЧасов;
		МассивЧасов = Новый Массив;
		Для К = 1 ПО МассивНовыхЗадач.Количество() Цикл
			СуммаРасходаНаЗадачу = Окр(СуммаРасходовНаКаждую * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
			МассивСумм.Добавить(СуммаРасходаНаЗадачу);
			СуммаЧасовНаЗадачу = Окр(СуммаЧасовВсего * СуммаРасходаНаЗадачу / СуммаРасходовВсего, 0);
			МассивЧасов.Добавить(СуммаЧасовНаЗадачу);
			СуммаРасходовВсего = СуммаРасходовВсего - СуммаРасходаНаЗадачу;
			СуммаЧасовВсего = СуммаЧасовВсего - СуммаЧасовНаЗадачу;
		КонецЦикла;
		Если СуммаРасходовВсего <> 0 И МассивНовыхЗадач.Количество() > 0 Тогда
			МассивСумм.Установить(0, МассивСумм.Получить(0) + СуммаРасходовВсего);
		КонецЕсли;
		Если СуммаЧасовВсего <> 0 И МассивНовыхЗадач.Количество() > 0 Тогда
			МассивЧасов.Установить(0, МассивЧасов.Получить(0) + СуммаЧасовВсего);
		КонецЕсли;
		Если РезультатЗакрытия.КоличествоСтрокРасходов = 0 Тогда
			РезультатЗакрытия.КоличествоСтрокРасходов = 1;
		КонецЕсли;
		Если РезультатЗакрытия.КоличествоСтрокВыплат = 0 Тогда
			РезультатЗакрытия.КоличествоСтрокВыплат = 1;
		КонецЕсли;
		Если РезультатЗакрытия.ВсегоРолей = 0 Тогда
			РезультатЗакрытия.ВсегоРолей = 1;
		КонецЕсли;
		Если РезультатЗакрытия.ВсегоРесурсов = 0 Тогда
			РезультатЗакрытия.ВсегоРесурсов = 1;
		КонецЕсли;
		Для К = 0 ПО МассивНовыхЗадач.Количество() - 1 Цикл
			ЭлементДерева = МассивНовыхЗадач.Получить(К);
			СтрокаТаблицыПроекта = тСоответствиеТаблицыУИД.Получить(ЭлементДерева.УИД);
			...
			МассивСуммПлана = Новый Массив;
			СуммаДоходовВсего = МассивСумм.Получить(К);
			СуммаДоходовНаКаждую = Окр(СуммаДоходовВсего / РезультатЗакрытия.КоличествоСтрокРасходов, 0);
			Для Ц = 1 ПО РезультатЗакрытия.КоличествоСтрокРасходов Цикл
				СуммаНаСтроку = Окр(СуммаДоходовНаКаждую * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
				МассивСуммПлана.Добавить(СуммаНаСтроку);
				СуммаДоходовВсего = СуммаДоходовВсего - СуммаНаСтроку;
			КонецЦикла;
			Если СуммаДоходовВсего <> 0 И МассивСуммПлана.Количество() > 0 Тогда
				МассивСуммПлана.Установить(0, МассивСуммПлана.Получить(0) + СуммаДоходовВсего);
			КонецЕсли;
			
			Для Ц = 0 ПО РезультатЗакрытия.КоличествоСтрокРасходов - 1 Цикл
				СтрокаПлана = ЭлементДерева.ПланРасходов.Добавить();
				СтрокаПлана.ГУИД = Новый УникальныйИдентификатор;
				СтрокаПлана.Сумма = МассивСуммПлана.Получить(Ц);
				СтрокаПлана.Валюта = ВалютаУчета;
				СтрокаПлана.СуммаУпр = СтрокаПлана.Сумма;
				
				СтрокаПлана.Статья = ПолучитьПроизвольныйЭлементМассива(МассивСтатейЗатрат, Генератор);
				СтрокаПлана.Организация = ПолучитьПроизвольныйЭлементМассива(МассивЗонРасчета, Генератор);
				СтрокаПлана.Подразделение = ПолучитьПроизвольныйЭлементМассива(МассивПодразделений, Генератор);
				СтрокаПлана.Номенклатура = ПолучитьПроизвольныйЭлементМассива(МассивНоменклатуры, Генератор);
				СтрокаПлана.ЗонаРасчета = ПолучитьПроизвольныйЭлементМассива(МассивЗонРасчета, Генератор);
				...
				//Иначе
					...
					//КонецЕсли;
				//КонецЕсли;
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПроекта.ПланРасходов.Добавить(), СтрокаПлана);
			КонецЦикла;
			
			МассивСуммПлана = Новый Массив;
			СуммаДоходовВсего = МассивСумм.Получить(К);
			СуммаДоходовНаКаждую = Окр(СуммаДоходовВсего / РезультатЗакрытия.КоличествоСтрокВыплат, 0);
			Для Ц = 1 ПО РезультатЗакрытия.КоличествоСтрокВыплат Цикл
				СуммаНаСтроку = Окр(СуммаДоходовНаКаждую * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
				МассивСуммПлана.Добавить(СуммаНаСтроку);
				СуммаДоходовВсего = СуммаДоходовВсего - СуммаНаСтроку;
			КонецЦикла;
			Если СуммаДоходовВсего <> 0 И МассивСуммПлана.Количество() > 0 Тогда
				МассивСуммПлана.Установить(0, МассивСуммПлана.Получить(0) + СуммаДоходовВсего);
			КонецЕсли;
			Для Ц = 0 ПО РезультатЗакрытия.КоличествоСтрокВыплат - 1 Цикл
				СтрокаПлана = ЭлементДерева.ПланДвиженияДенежныхСредств.Добавить();
				СтрокаПлана.ГУИД = Новый УникальныйИдентификатор;
				СтрокаПлана.Направление = ПредопределенноеЗначение("Перечисление.грзнтНаправлениеДвиженияДенежныхСредств.Выплата");
				СтрокаПлана.Сумма = МассивСуммПлана.Получить(Ц);
				СтрокаПлана.Валюта = ВалютаУчета;
				СтрокаПлана.СуммаУпр = СтрокаПлана.Сумма;
			...
			МассивЧасовПлана = Новый Массив;
			МассивСуммПлана = 0;
			СуммаЧасовВсего = МассивЧасов.Получить(К);
			СуммаЧасовНаКаждую = Окр(СуммаЧасовВсего / РезультатЗакрытия.ВсегоРолей, 0);
			СуммаРасходовВсего = МассивСумм.Получить(К);
			Для Ц = 1 ПО РезультатЗакрытия.ВсегоРолей Цикл
				СуммаЧасовНаСтроку = Окр(СуммаЧасовНаКаждую * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
				МассивЧасовПлана.Добавить(СуммаЧасовНаСтроку);
				//СуммаНаСтроку = СуммаРасходовВсего * СуммаЧасовНаСтроку / СуммаЧасовВсего;
				//МассивСуммПлана.Добавить(СуммаНаСтроку);
				СуммаЧасовВсего = СуммаЧасовВсего - СуммаЧасовНаСтроку;
			КонецЦикла;
			Если СуммаЧасовВсего <> 0 И МассивЧасовПлана.Количество() > 0 Тогда
				МассивЧасовПлана.Установить(0, МассивЧасовПлана.Получить(0) + СуммаЧасовВсего);
			КонецЕсли;
			НовыйГенератор = Новый ГенераторСлучайныхЧисел(МассивРолей.Количество() * (К + 1));
			Для Ц = 0 ПО РезультатЗакрытия.ВсегоРолей - 1 Цикл
				СтрокаПлана = ЭлементДерева.ПланРолейОбщая.Добавить();
				СтрокаПлана.ГУИД = Новый УникальныйИдентификатор;
				СтрокаПлана.Количество = МассивЧасовПлана.Получить(Ц);
				СтрокаПлана.ЕдиницаВремени = ПредопределенноеЗначение("Справочник.грзнтЕдиницыВремени.Час");
				СтрокаПлана.Роль = ПолучитьПроизвольныйЭлементМассива(МассивРолей, НовыйГенератор);
				...
				//Иначе
					...
					//КонецЕсли;
				//КонецЕсли;
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПроекта.ПланРолейОбщая.Добавить(), СтрокаПлана);
			КонецЦикла;
			
			МассивЧасовПлана = Новый Массив;
			МассивСуммПлана = 0;
			СуммаЧасовВсего = МассивЧасов.Получить(К);
			СуммаЧасовНаКаждую = Окр(СуммаЧасовВсего / РезультатЗакрытия.ВсегоРесурсов, 0);
			СуммаРасходовВсего = МассивСумм.Получить(К);
			Для Ц = 1 ПО РезультатЗакрытия.ВсегоРесурсов Цикл
				СуммаЧасовНаСтроку = Окр(СуммаЧасовНаКаждую * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
				МассивЧасовПлана.Добавить(СуммаЧасовНаСтроку);
				//СуммаНаСтроку = СуммаРасходовВсего * СуммаЧасовНаСтроку / СуммаЧасовВсего;
				//МассивСуммПлана.Добавить(СуммаНаСтроку);
			...
			КонецЕсли;
			НовыйГенератор = Новый ГенераторСлучайныхЧисел(МассивРолей.Количество() * (К + 1));
			Для Ц = 0 ПО РезультатЗакрытия.ВсегоРесурсов - 1 Цикл
				СтрокаПлана = ЭлементДерева.ПланРесурсовОбщая.Добавить();
				СтрокаПлана.ГУИД = Новый УникальныйИдентификатор;
				СтрокаПлана.Количество = МассивЧасовПлана.Получить(Ц);
				СтрокаПлана.ЕдиницаВремени = ПредопределенноеЗначение("Справочник.грзнтЕдиницыВремени.Час");
				СтрокаПлана.Роль = ПолучитьПроизвольныйЭлементМассива(МассивРолей, НовыйГенератор);
				СтрокаПлана.ТрудовойРесурс = ПолучитьПроизвольныйЭлементМассива(МассивРесурсов, Генератор);
				...
				//Иначе
					...
					//КонецЕсли;
				//КонецЕсли;
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПроекта.ПланРесурсовОбщая.Добавить(), СтрокаПлана);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	
	Если РезультатЗакрытия.ОбщееКоличествоНоменклатуры > 0 Тогда
		НоменклатурыВсего = РезультатЗакрытия.ОбщееКоличествоНоменклатуры;
		НоменклатурыНаКаждую = Окр(НоменклатурыВсего / МассивНовыхЗадач.Количество(), 0);
		МассивСумм = Новый Массив;
		Для К = 1 ПО МассивНовыхЗадач.Количество() Цикл
			НоменклатурыНаЗадачу = Окр(НоменклатурыНаКаждую * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
			МассивСумм.Добавить(НоменклатурыНаЗадачу);
			НоменклатурыВсего = НоменклатурыВсего - НоменклатурыНаЗадачу;
		КонецЦикла;
		Если НоменклатурыВсего <> 0 И МассивНовыхЗадач.Количество() > 0 Тогда
			МассивСумм.Установить(0, МассивСумм.Получить(0) + НоменклатурыВсего);
		КонецЕсли;
		Для К = 0 ПО МассивНовыхЗадач.Количество() - 1 Цикл
			ЭлементДерева = МассивНовыхЗадач.Получить(К);
			СтрокаТаблицыПроекта = тСоответствиеТаблицыУИД.Получить(ЭлементДерева.УИД);
			...
			МассивСуммПлана = Новый Массив;
			СуммаДоходовВсего = МассивСумм.Получить(К);
			СуммаДоходовНаКаждую = Окр(СуммаДоходовВсего / 1, 0);
			Для Ц = 1 ПО 1 Цикл
				СуммаДоходаНаСтроку = Окр(СуммаДоходовНаКаждую * Генератор.СлучайноеЧисло(70, 130) / 100, 0);
				МассивСуммПлана.Добавить(СуммаДоходаНаСтроку);
				СуммаДоходовВсего = СуммаДоходовВсего - СуммаДоходаНаСтроку;
			...
			Для Ц = 0 ПО 1 - 1 Цикл
				СтрокаПлана = ЭлементДерева.ПланМатериаловОбщая.Добавить();
				СтрокаПлана.ГУИД = Новый УникальныйИдентификатор;
				СтрокаПлана.Количество = МассивСуммПлана.Получить(Ц);
				...
				СписокВыбора = ПолучитьСписокВыбораЕдиницыИзмерения(СтрокаПлана.Номенклатура);
				Если СписокВыбора.Количество() > 0 Тогда
					СтрокаПлана.ЕдиницаИзмерения = СписокВыбора.Получить(0).Значение;
				КонецЕсли;
				//СлучайноеЧисло = Генератор.СлучайноеЧисло(1, 2);
				//Если СлучайноеЧисло = 1 Тогда 
				//	СтрокаПлана.СпособПривязки = ПредопределенноеЗначение("Перечисление.грзнтСпособПривязки.НаДату");
				...
				//Иначе
					...
					//КонецЕсли;
				//КонецЕсли;
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПроекта.ПланМатериаловОбщая.Добавить(), СтрокаПлана);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Функция ПолучитьПроизвольныйЭлементМассива(МассивЭлементов, Генератор)
	Если МассивЭлементов.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	СлучайноеЧисло = Генератор.СлучайноеЧисло(1, МассивЭлементов.Количество());
	Возврат МассивЭлементов.Получить(СлучайноеЧисло - 1);
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьМассивРолейДляДемо()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	грзнтРоли.Ссылка
	|ИЗ
	|	Справочник.грзнтРоли КАК грзнтРоли
	|ГДЕ
	|	НЕ грзнтРоли.ЭтоГруппа
	|	И НЕ грзнтРоли.ПометкаУдаления";
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьМассивРесурсовДляДемо()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	грзнтТрудовыеРесурсы.Ссылка
	|ИЗ
	|	Справочник.грзнтТрудовыеРесурсы КАК грзнтТрудовыеРесурсы
	|ГДЕ
	|	НЕ грзнтТрудовыеРесурсы.ПометкаУдаления
	|	И НЕ грзнтТрудовыеРесурсы.ЭтоГруппа";
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьМассивНоменклатурыДляДемо()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Номенклатура.Ссылка
...
&НаСервереБезКонтекста
Функция ПолучитьМассивОрганизацийДляДемо()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Спр.Ссылка
...
&НаСервереБезКонтекста
Функция ПолучитьМассивПодразделенийДляДемо()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Спр.Ссылка
...
&НаСервереБезКонтекста
Функция ПолучитьМассивСтатейЗатратДляДемо()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Спр.Ссылка
...
&НаСервереБезКонтекста
Функция ПолучитьМассивСтатейДоходовДляДемо()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Спр.Ссылка
	|ИЗ
	|	Справочник.грзнтСтатьиДоходов КАК Спр
	|ГДЕ
	|	НЕ Спр.ПометкаУдаления
	|	И НЕ Спр.ЭтоГруппа";
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
...
&НаСервереБезКонтекста
Функция ПолучитьМассивСтатейДДСДляДемо()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Спр.Ссылка
	|ИЗ
	|	Справочник.СтатьиДвиженияДенежныхСредств КАК Спр
	|ГДЕ
	|	НЕ Спр.ПометкаУдаления
	|	И НЕ Спр.ЭтоГруппа";
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
...
&НаСервереБезКонтекста
Функция ПолучитьМассивКонтрагентовДляДемо()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Спр.Ссылка
...
&НаСервереБезКонтекста
Функция ПолучитьМассивЗонРасчетаДляДемо()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Спр.Ссылка
	|ИЗ
	|	Справочник.грзнтЗоныРасчета КАК Спр
	|ГДЕ
	|	НЕ Спр.ПометкаУдаления";
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
КонецФункции
