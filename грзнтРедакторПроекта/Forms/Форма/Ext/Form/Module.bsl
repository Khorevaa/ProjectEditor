...
Функция РекурсивныйПересчетПолучитьСтруктуруТаблицПоУмолчанию()
	...
	СтруктураТаблиц.Вставить("УчитыватьФакт", Истина);
...
Процедура ЗаполнитьДемоДанныеПродолжение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия.СоздаватьНовые Тогда
		...
		МассивИзменений = Новый Массив;
		ИдентификаторФормы = ЭтаФорма.УникальныйИдентификатор;
		АдресВременногоХранилищаДанныхНаСервере = РекурсивныйПересчетПриИзмененииДерева(АдресВременногоХранилищаДанныхНаСервере, ИдентификаторФормы, МассивСообщенийНаСервер, МассивИзменений, РежимУчитыватьФакт);
		ОбновитьТаблицуПроектаПоМассивуИзменений(МассивИзменений, Ложь);
