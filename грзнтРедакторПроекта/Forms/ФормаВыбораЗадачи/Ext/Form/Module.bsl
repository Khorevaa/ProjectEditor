...
Процедура ПриОткрытии(Отказ)
	...
	Для Каждого СтрокаСвязи ИЗ ТекущийВладелец.ТаблицаПредшественники Цикл
		...
		// пока непонятно зачем, но ограничим на всякий случай
		Если ОперацияВыбора <> "ВыборПредшественника" Тогда
...
Процедура КомандаВыбрать(Команда)
	...
	ИначеЕсли ОперацияВыбора = "ВыборЗадачиИсточникаЗаполнения" Тогда
		...
		Если МассивПредшественники.Количество() > 0 Тогда
			...
			ТекущийВладелец.АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПроверитьВозможностьЗаменыПредшественников(ТекущийВладелец.АдресВременногоХранилищаДанныхНаСервере, ТекущийВладелец.УникальныйИдентификатор, МассивТекущие, МассивПредшественники, РезультатПроверки);
	...
	ИначеЕсли ОперацияВыбора = "ВыборРодителя" Тогда
		...
		КонецЕсли;
		РезультатПроверки = "";
		ТекущийВладелец.АдресВременногоХранилищаДанныхНаСервере = грзнтРекурсивныйПересчет.РекурсивныйПересчетПроверитьВозможностьПеремещения(ТекущийВладелец.АдресВременногоХранилищаДанныхНаСервере, ТекущийВладелец.УникальныйИдентификатор, ТекущийУИД, ВыбранноеЗначение, РезультатПроверки);
		Если НЕ ПустаяСтрока(РезультатПроверки) Тогда
